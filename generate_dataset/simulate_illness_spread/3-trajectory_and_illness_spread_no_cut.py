...
init = args.init
graph_save_dir = '../../data/{0}/graphs/no_cut'.format(Network)
...
P_t_raw = loadmat("../../data/{0}/region_file/rhythm.mat".format(Network))['rhythm']  # 加载Pt数据，也就是出行的昼夜节律
...
def individual_trace_simulate(info, p_t, current_location_type, now_time):
    # region ID, time_slot
...
def simulate_trace(hour, start_time, pop_info):
...
def trace_and_illness_spread_simulate(pop_num, hour_length):
    ...
    feature_graph = np.load('../../data/{0}/trajectories/original.npy'.format(Network))
    for hour in range(Simu_slot):  # 对于每一个小时
        day = hour // 24
        if day < 7:
            simulate_trace(hour, start_time, pop_info)
        else:
            simulate_trace(hour, start_time, pop_info)
...
output = open('{0}/pop_info.pkl'.format(trajectories_dir), 'wb')
